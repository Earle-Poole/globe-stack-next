...
const CoordsCopier = (props: CoordsCopierProps) => {
  ...
  const mouseMoveHandler = useRef(
    throttle((e: any) => {
      const tooltipWrapperOffset = tooltipWrapperRef.current.offsetWidth
      const { x, y } = e.pixel
      setLocation({
        x:
          x - (tooltipWrapperOffset / 2 + 5) < 0
            ? 5
            : x + (tooltipWrapperOffset / 2 + 5) > window.innerWidth
            ? window.innerWidth - (tooltipWrapperOffset + 5)
            : x - tooltipWrapperOffset / 2,
        y: y > window.innerHeight - 105 ? y - 85 : y + 25,
        ...e.latLng.toJSON(),
      })
    }, 10)
  )
  ...
  useEffect(() => {
    if (primaryMap) {
      ...
      const moveListener = primaryMap.addListener(
        'mousemove',
        mouseMoveHandlerRef
      )
  ...
  return (
    <CoordsCopierTooltipWrapper
      ...
      style={{
        left: location.x.toString() + 'px',
        top: location.y.toString() + 'px',
      }}
      location={location}>
