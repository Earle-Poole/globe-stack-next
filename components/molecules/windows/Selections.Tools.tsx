...
export const Polyline = () => {
  ...
  useEffect(() => {
    ...
    const updateLength = () => {
      setLength(
        google.maps.geometry.spherical.computeLength(currentPolyline.getPath())
      )
    }
...
export const Polygon = () => {
  ...
  const handleEdit = () => {
    const editPath: google.maps.MVCArray<google.maps.LatLng> =
      polygon.current.getPath()
    const bounds = new google.maps.LatLngBounds()
...
export const Rectangle = () => {
  const [firstCorner, setFirstCorner] = useState<google.maps.LatLng | null>(
    null
  )
  const [secondCorner, setSecondCorner] = useState<google.maps.LatLng | null>(
    null
  )
  const [area, setArea] = useState(0)
  const [center, setCenter] = useState(new google.maps.LatLng(0, 0))
  ...
  useEffect(() => {
    ...
    const mouseDownListener = primaryMap.addListener(
      'mousedown',
      mouseDownHandler
    )
    ...
    if (ne && sw) {
      ...
      boundsChanged = currentRectangle.addListener(
        'bounds_changed',
        handleEdit(path)
      )
